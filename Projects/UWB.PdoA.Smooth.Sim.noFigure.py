...
class CircularDataGenerator:
    ...
    def __init__(self, radius=100, center_x=0, center_y=0, angular_speed=1):
        self.radius = radius  # 圆的半径(cm)
        self.center_x = center_x  # 圆心X坐标
        self.center_y = center_y  # 圆心Y坐标
        self.angular_speed = angular_speed  # 角速度(rad/s)
        self.start_time = time.time()

    def get_data(self):
        """生成一个数据点"""
        current_time = time.time() - self.start_time
        angle = self.angular_speed * current_time
...
if __name__ == '__main__':
    ...
    # 创建模拟数据生成器
    data_generator = CircularDataGenerator(radius=100, angular_speed=1)
